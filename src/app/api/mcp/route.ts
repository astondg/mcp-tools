      ...
      async (params) => {
        try {
          ...
          const lines = goalsWithProgress.map(({ goal, progress }) => {
            const status = goal.isCompleted ? '✅' : (progress?.isOnTrack ? '🟢' : '🟡');
    ...
    server.tool(
      ...
      async (params) => {
        try {
          ...
          return {
            content: [{ type: 'text', text: `🏋️ Training Programs (${programs.length}):\n\n${lines.join('\n\n')}` }]
          };
        } catch (error) {
    ...
    server.tool(
      ...
      async () => {
        try {
          ...
          let output = `🏋️ Active Program: ${active.program.name}\n\n`;
      ...
      async () => {
        try {
          ...
          if (summary.activeProgram) {
            output += `🏋️ Active Program: ${summary.activeProgram.name}\n`;
          ...
          if (summary.goals.length > 0) {
            ...
            for (const goal of summary.goals) {
              const status = goal.isCompleted ? '✅' : (goal.isOnTrack ? '🟢' : '🟡');
